  ...
  public  class CoursRepository
    {
      ...
      public DataTable GetAllksanayeList()
      {
          List<ksanaye> result = new List<ksanaye>();

          using (hadafDBEntities DC = conn.GetContext())
          {
              IEnumerable<ksanaye> pl =
                  from r in DC.ksanayes
                  orderby r.id
                  select r;
      ...
      public DataTable GetAllKarMList()
      {
          List<KarM> result = new List<KarM>();
